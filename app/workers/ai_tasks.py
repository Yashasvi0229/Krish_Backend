...
async def _find_or_create_claim(
    ...
    if client_id is None:
        # Race guard: two concurrent syncs both find no placeholder and
        # both INSERT — resulting in duplicate "Unassigned" clients.
        # Postgres advisory locks let us serialize just this creation
        # step without blocking other tables. The lock is transaction-
        # scoped (released on commit/rollback), so we don't have to
        # remember to unlock. Any 64-bit int works as the key — we pick
        # a stable one so all workers hash to the same lock.
        from sqlalchemy import text
        await session.execute(text("SELECT pg_advisory_xact_lock(9001001)"))
